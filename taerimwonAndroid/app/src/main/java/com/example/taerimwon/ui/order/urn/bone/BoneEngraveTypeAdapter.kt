package com.example.taerimwon.ui.order.urn.bone
...
class BoneEngraveTypeAdapter(private val context: Context): RecyclerView.Adapter<BoneEngraveTypeAdapter.BoneEngraveTypeListViewHolder>() {
    private var boneEngraveTypeList = mutableListOf<String>()
    lateinit var onItemClickListener: (View, String) -> Unit
    // 클릭된 아이템의 위치를 저장하는 변수
    private var selectedItemPosition = 0

    fun setListDate(data: MutableList<String>){
        boneEngraveTypeList = data
    }
    ...
    fun setSelectedItem(position: Int) {
        ...
        ApplicationClass.prefs.boneEngraveTypePosition = position
        println("boneEngraveTypePosition: " + ApplicationClass.prefs.boneEngraveTypePosition)

        notifyDataSetChanged() // 변경 사항을 적용하기 위해 어댑터에 알립니다.
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BoneEngraveTypeListViewHolder {
        return BoneEngraveTypeListViewHolder(
    ...
    override fun onBindViewHolder(holder: BoneEngraveTypeListViewHolder, position: Int) {
        holder.bind(boneEngraveTypeList[position])
    ...
    override fun getItemCount(): Int {
        return boneEngraveTypeList.size
    }

    inner class BoneEngraveTypeListViewHolder(private val binding: ItemEngraveTypeBinding, private val context: Context)
        : RecyclerView.ViewHolder(binding.root) {
        lateinit var boneEngraveType: String
