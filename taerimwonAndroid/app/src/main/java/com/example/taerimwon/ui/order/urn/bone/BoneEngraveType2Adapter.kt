...
class BoneEngraveType2Adapter(private val context: Context): RecyclerView.Adapter<BoneEngraveType2Adapter.BoneEngraveType2ListViewHolder>() {
    private var boneEngraveType2List = mutableListOf<String>()
    lateinit var onItemClickListener: (View, String) -> Unit
    // 클릭된 아이템의 위치를 저장하는 변수
    private var selectedItemPosition = 0

    fun setListDate(data: MutableList<String>){
        boneEngraveType2List = data
    }
    ...
    fun setSelectedItem(position: Int) {
        ...
        ApplicationClass.prefs.boneEngraveType2Position = position
        notifyDataSetChanged() // 변경 사항을 적용하기 위해 어댑터에 알립니다.
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BoneEngraveType2ListViewHolder {
        return BoneEngraveType2ListViewHolder(
    ...
    override fun onBindViewHolder(holder: BoneEngraveType2ListViewHolder, position: Int) {
        holder.bind(boneEngraveType2List[position])
    ...
    override fun getItemCount(): Int {
        return boneEngraveType2List.size
    }

    inner class BoneEngraveType2ListViewHolder(private val binding: ItemEngraveTypeBinding, private val context: Context)
        : RecyclerView.ViewHolder(binding.root) {
        lateinit var boneEngraveType2: String
